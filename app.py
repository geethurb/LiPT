...
if uploaded_file and st.button(":orange[**Train Model and Generate Molecules**]"):
    lipid_data = pd.read_csv(uploaded_file)
    if 'smiles' not in lipid_data.columns:
        st.error("Uploaded CSV must include a 'smiles' column.")
        st.stop()

    lipid_data = lipid_data.dropna(subset=['smiles']).drop_duplicates(subset=['smiles']).copy()
    lipid_data['smiles'] = lipid_data['smiles'].astype(str).apply(canonicalize)
    lipid_data = lipid_data.dropna().sample(frac=1, random_state=42).reset_index(drop=True)

    if lipid_data.empty:
        st.error("No valid SMILES were found after cleaning.")
        st.stop()
    ...
    if vocab_size < 2:
        st.error("Not enough unique tokens to train. Upload a more diverse SMILES dataset.")
        st.stop()
    ...
    n = len(data)
    train_text = data[:int(n * split_ratio)]
    val_text = data[int(n * split_ratio):]
    train_ids = encode(train_text)
    val_ids = encode(val_text)
    block_size_i = int(block_size)
    batch_size_i = int(batch_size)
    min_tokens_required = block_size_i + 1
    if len(train_ids) < min_tokens_required or len(val_ids) < min_tokens_required:
        st.error(
            f"Not enough tokens for block size {block_size_i}. "
            f"Need at least {min_tokens_required} tokens in both train and validation splits."
        )
        st.stop()
    st.write(f"{len(train_ids):,} tokens for training")
    st.write(f"{len(val_ids):,} tokens for testing")
    st.write(":orange[Patience is key. Training may take a while.]")
    ...
    def get_batch(split):
        ...
        max_start = len(split_data) - block_size_i
        ix = torch.randint(max_start, (batch_size_i,))
        x = torch.stack([torch.from_numpy((split_data[i:i+block_size_i]).astype(np.int64)) for i in ix])
        y = torch.stack([torch.from_numpy((split_data[i+1:i+1+block_size_i]).astype(np.int64)) for i in ix])
        if device_type == 'cuda':
    ...
    model_args = dict(n_layer=n_layer, n_head=n_head, n_embd=n_embd, block_size=block_size_i, bias=bias, vocab_size=None, dropout=dropout)
    model_args['vocab_size'] = meta_vocab_size
    gptconf = GPTConfig(**model_args)
    model = GPT(gptconf)
    if block_size_i < model.config.block_size:
        model.crop_block_size(block_size_i)
        model_args['block_size'] = block_size_i
    model.to(device)
    ...
    while True:
        ...
        if iter_num % log_interval == 0 and master_process:
            ...
            if local_iter_num >= 5: # let the training loop settle a bit
                mfu = raw_model.estimate_mfu(batch_size_i * gradient_accumulation_steps, dt)
    ...
    start_token = "\n" if "\n" in stoi else chars[0]
    max_new_tokens = 512 # number of tokens generated in each sample
    top_k = min(100, vocab_size) # retain only the top_k most likely tokens, clamp others to have 0 probability
    x = torch.tensor([[stoi[start_token]]], dtype=torch.long, device=device)
    gen_smiles = []
    target_samples = int(num_samples)
    max_generation_attempts = target_samples * 20
    generation_attempts = 0
    with torch.no_grad():
        with ctx:
            while len(gen_smiles) < target_samples and generation_attempts < max_generation_attempts:
    ...
    if smiles_df.empty:
        st.warning("No valid novel molecules were generated. Try more iterations or a higher temperature.")
    else:
        st.table(smiles_df)
